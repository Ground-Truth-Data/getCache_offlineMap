<script lang="ts">
/** Bytes off the network, by feature, per day; survives reloads, unlike the session dock. */
import { onMount } from "svelte";
import { dailyAverage, pastDays, startDataMeter, todayBytes, todayTotal } from "../../lib/shared/dataMeter.svelte";

// Not `$effect`: starting the meter writes the reactive map this component reads, which Svelte stops as `effect_update_depth_exceeded`.
...
<div class="data dev-card">
	...
	<!-- transferSize is 0 for a cache hit and for a cross-origin response with no Timing-Allow-Origin. -->
