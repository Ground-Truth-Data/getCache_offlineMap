...
export function featureToKML(feature: Feature): string {
    ...
    const desc =
        ...
        "";
    // Every row here shows up verbatim under the pin in Google My Maps
    // (see shareWire.ts) — the machine fields ride as human labels, and
    // normaliseProps reads them back on import. Foreign KML never carries
    // the stamp, so a re-import stays unstamped; our drawn pins stay "ours"
    // through any number of sends.
    const pinTypeKey = (feature.properties?.pinTypeKey as string | undefined) || "";
    const stamped = feature.properties?.madeWith === MADE_WITH_GETCACHE;
    const featureData =
    ...
    const row = (name: string, value: string): string =>
        value
            ? `<Data name="${name}"><value>${escapeXml(value)}</value></Data>`
            : "";
    const dataItems = [
        row(WIRE.pin, pinTypeKey),
        row(WIRE.type, isTrack ? "track" : ""),
        row(WIRE.madeWith, stamped ? MADE_WITH_GETCACHE : ""),
        row(WIRE.data, featureData ? recordsToCsv(featureData) : ""),
        row(WIRE.contacts, contactsData ? contactsToLines(contactsData) : ""),
    ].join("");
...
// Shares made before 7 Sep 2026 carried the styled Google-Earth card in
// <description> (it now rides in the placemark's BalloonStyle), with the
// real notes in ExtendedData `featureDesc`. Such a description is
// presentation-only HTML and must be DROPPED, never bridged into notes.
const RT_BALLOON_MARK = "<!--rt-balloon-->";

// The human-labelled wire rows (shareWire.ts) → the app's property names.
// Shares written before the labels landed carry the property names
// directly (`pinTypeKey`, `featureData` as JSON, …) and pass through
// untouched; a label only ever fills a property that is still empty. The
// one exception is the stamp: its pre-2026-09-07 form `isRetreever` =
// "isRetreever" is translated to `madeWith`, since the cell was renamed.
function readWireRows(props: Record<string, unknown>): void {
    const take = (label: string): string | null => {
        const v = props[label];
        delete props[label];
        return typeof v === "string" && v.trim() ? v : null;
    };
    const pin = take(WIRE.pin);
    if (pin && props.pinTypeKey == null) props.pinTypeKey = pin.trim();
    const type = take(WIRE.type);
    if (type && props.featureType == null) props.featureType = type.trim();
    const legacyStamp = take("isRetreever") === "isRetreever";
    if (
        (take(WIRE.madeWith)?.trim() === MADE_WITH_GETCACHE || legacyStamp) &&
        props.madeWith == null
    ) {
        props.madeWith = MADE_WITH_GETCACHE;
    }
    const data = take(WIRE.data);
    if (data && props.featureData == null) {
        const json = csvToRecordsJson(data);
        if (json) props.featureData = json;
    }
    const contacts = take(WIRE.contacts);
    if (contacts && props.contactsData == null) {
        const json = linesToContactsJson(contacts);
        if (json) props.contactsData = json;
    }
}
