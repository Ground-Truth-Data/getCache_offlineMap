// ⛔ Roads sat 45.2 km off the pin (grid-tile addressing) vs the satellite blob's 3 m (stores its own bounds) — these tests keep the box centred on the pin.
...
describe("the pin is the centre", () => {
	...
	it("longitude is corrected for latitude — or the box is too narrow", () => {
		// Skipping cos(lat) makes the box ~20 km wide instead of 60 (longitude shrinks away from the equator).
		const b = boxAround(CHELAN.lng, CHELAN.lat, 30);
		const { widthKm, heightKm } = sizeKm(b);
	...
	it("refuses bad input loudly instead of emitting NaN", () => {
		// A NaN coordinate red-screens the map — fail at the source, never downstream.
