...
export function wallLabelLayers(
	...
	return [
		// ── TOWN LABELS ──────────────────────────────────────────────────────
		// GRANULAR places: city / town / village / hamlet by KIND_DETAIL. Suburb,
		// neighbourhood, locality and region noise is dropped by simply not
		// listing them.
		//
		// ⛔ `kind_detail`, NOT `kind`. Protomaps v4 tags EVERY places feature
		// `kind:"locality"` and files city/town/village/hamlet under
		// `kind_detail` — MEASURED 28 Aug 2026, 214/214 features in one disc.
		// This filter read `kind`, so it matched nothing, and the Worker's old
		// allowlist made the same mistake, so the pack carried nothing for it to
		// miss. Both now read the contract's key (contract/packLayers.ts).
		//
