...
/** ⛔ Table and cell names are the caller's — never hardcode app-specific names here. */
/** ⚠️ Must run before the persister loads — the schema no longer declares the old cell, so a load drops it unmoved. */
export interface PointerFlip {
	/** The table + cell that USED to hold the pointer (a row id in `to.table`). */
	from: { table: string; cell: string };
	/** The table + cell that holds it now: `to.table[<old value>][to.cell] = <from row id>`.
	 *  One cell per target row — the first source row (by `orderBy`, then id) wins. */
	to: { table: string; cell: string };
	/** Source cell to order rivals by (ascending) — an ISO stamp works. */
	orderBy?: string;
}

/** Turn a many-to-one pointer into its one-to-one inverse at the raw
 *  IndexedDB level: every `from` row naming a `to` row hands that row its
 *  own id, and the old cell is deleted from every `from` row whether or
 *  not it found a home. No-ops once nothing carries the old cell. */
export async function invertPointerInIdb(
	dbName: string,
	flip: PointerFlip,
): Promise<void> {
	if (typeof indexedDB === "undefined") return;
	try {
		...
		await new Promise<void>((resolve) => {
			...
			type Rec = { k: string; v: Record<string, Record<string, unknown>> };
			const getFrom = os.get(flip.from.table);
			getFrom.onsuccess = () => {
				const fromRec = getFrom.result as Rec | undefined;
				if (!fromRec) return;
				const fromRows = fromRec.v ?? {};
				const carriers = Object.entries(fromRows)
					.filter(
						([, row]) =>
							row && typeof row === "object" && flip.from.cell in row,
					)
					.sort(([ia, a], [ib, b]) => {
						const oa = flip.orderBy ? String(a[flip.orderBy] ?? "") : "";
						const ob = flip.orderBy ? String(b[flip.orderBy] ?? "") : "";
						return oa < ob ? -1 : oa > ob ? 1 : ia < ib ? -1 : ia > ib ? 1 : 0;
					});
				if (carriers.length === 0) return;
				const getTo = os.get(flip.to.table);
				getTo.onsuccess = () => {
					const toRec = (getTo.result as Rec | undefined) ?? {
						k: flip.to.table,
						v: {},
					};
					const toRows = toRec.v ?? {};
					for (const [fromId, row] of carriers) {
						const target = row[flip.from.cell];
						delete row[flip.from.cell];
						dropped++;
						if (typeof target !== "string" || !target) continue;
						const dest = toRows[target];
						if (!dest || typeof dest !== "object") continue;
						if (dest[flip.to.cell]) continue;
						dest[flip.to.cell] = fromId;
						moved++;
					}
					os.put({ k: flip.from.table, v: fromRows });
					if (moved) os.put({ k: flip.to.table, v: toRows });
				};
			};
		...
		if (dropped) {
			console.info(
				`[idbRename] flipped ${flip.from.table}.${flip.from.cell} → ${flip.to.table}.${flip.to.cell}: ${moved} pointer(s) moved, ${dropped} old cell(s) dropped`,
			);
		}
	} catch {
		// Best-effort: never let the flip break boot — old cells stay for retry.
	}
}
